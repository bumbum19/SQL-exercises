...
WITH t AS
(SELECT product_id, YEAR(purchase_date) current_year, lead(YEAR(purchase_date)) OVER (PARTITION BY product_id ORDER BY YEAR(purchase_date) ) next_year
FROM orders GROUP BY 1,2 HAVING COUNT(*) >=3)

SELECT DISTINCT product_id FROM t WHERE next_year - current_year = 1
