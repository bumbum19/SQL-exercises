...
WITH t AS 
(SELECT team_id, name, points, points + points_change new_points, 
CONVERT( RANK() OVER (ORDER BY points DESC, name), SIGNED) ranking,
CONVERT(RANK() OVER (ORDER BY points + points_change DESC, name),SIGNED) new_ranking
FROM TeamPoints NATURAL JOIN PointsChange
)

SELECT team_id, name, ranking - new_ranking rank_diff FROM t
