...
WITH t AS 
(SELECT product_id, order_id, order_date, RANK() over w order_rank FROM orders WINDOW w AS (PARTITION BY product_id ORDER BY order_date DESC) 
)

SELECT product_name, product_id, order_id, order_date FROM t NATURAL JOIN products WHERE order_rank = 1 ORDER BY 1,2,3
